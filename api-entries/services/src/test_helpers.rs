...
lazy_static! {
    pub static ref TEST_KEYS_1: (PublicKey, PrivateKey) = parse_keys(PUBLIC_KEY_1, PRIVATE_KEY_1);
}

lazy_static! {
    pub static ref TEST_KEYS_2: (PublicKey, PrivateKey) = parse_keys(PUBLIC_KEY_2, PRIVATE_KEY_2);
}

fn parse_keys(public: &str, private: &str) -> (PublicKey, PrivateKey) {
    let public_key =
...
pub fn test_timepoint() -> Box<dyn TimeProvider + Sync + Send> {
    Box::new(TimeStatic::new(1662750865))
}
...
pub fn test_payload(
    ...
    PayloadBytes::encrypt(public_key, private_key, timestamp, plaintext).unwrap()
}

pub async fn items_plaintext(
